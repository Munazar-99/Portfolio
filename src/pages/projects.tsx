...
const FeaturedProject = ({  title, summary, image, link, github }: {  title: string, summary: string, image: string, link: string, github: string }) => {
    return (
        <div className='w-full flex items-center justify-between ronded-br-2xl relative rounded-3xl border border-solid border-dark bg-light shadow-2xl p-12 dark:bg-dark dark:border-light lg:flex-col lg:p-8 xs:rounded-2xl xs:rounded-br-3xl  xs:p-4 '>
            ...
            <div className='w-1/2 flex flex-col items-start justify-between pl-6 lg:w-full lg:pl-0 lg:pt-6'>
                <span className='text-primary font-medium text-xl dark:text-primaryDark xs:text-base'>Featured Projects</span>
                        ...
                        {projects.map((project, i) =>{
                            ...
                            return (
                                i%3==0 ? (
                                    ...
                                    <FeaturedProject title={project.title} summary={project.summary} link={project.linktobuild}  github={project.linktogithub} image={urlFor(project.image).url()} />
                                </article>
                                ) : (
                                    <article key={project._id} className='col-span-6 sm:col-span-12'>
                            <Project title={project.title} summary={project.summary} link={project.linktobuild}  github={project.linktogithub} image={urlFor(project.image).url()} />
                        </article>
                                )
                            )
                        })}
